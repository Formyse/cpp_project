...
char ifieldlen[MAXFIELDCOUNT];         // 结果集字段的长度数组，从starg.fieldlen解析得到。
...
bool readincfield();   // 从s数据库表中或starg.incfilename文件中获取已抽取数据的最大id。
...
int main(int argc,char *argv[])
{
  ...
  //PActive.AddPInfo(5000,starg.pname); // 调试阶段防止超时。
...
bool _dminingmysql()
{
  ...
  while (true)
  {
    ...
    if (File.IsOpened()==false)
    {
      ...
      {
        logfile.Write("File.OpenAndRename(%s) failed.\n",strxmlfilename);
      }
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  if (CmdStr.CmdCount()>MAXFIELDCOUNT)
  {
    logfile.Write("fieldstr的字段数太多，超出的最大限制%d。\n",MAXFIELDCOUNT); return false;
  }
  ...
  // 判断strfieldnaem和ifieldlen两个数组中的字段是否一致。
...
bool instarttime()
{
  // 程序运行的时间区间，例如02，13表示；如果程序启动时，踏中02时和13时则运行，其他时间不运行。
...
// 把已抽取数据的最大id写入数据库或starg.incfilename文件。
bool writeincfield()
{
  // 如果starg.incfiled参数为空，表示不是增量抽取。
  if (strlen(starg.incfield)==0) return true;
