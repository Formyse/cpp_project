...
bool CZHOBTMIND::SplitBuffer(char *strBuffer,bool isxml)
{
  ...
  if (isxml==true)
  {
    ...
    GetXMLBuffer(strBuffer,"u",tmp,10);

    GetXMLBuffer(strBuffer,"wd",tmp,10);
  ...
  else
  {
    ...
    CmdStr.GetValue(4,tmp,10);

    CmdStr.GetValue(5,tmp,10);
...
bool CZHOBTMIND::InsertTable()
{
  ...
  { 
    ...
    // 答：如果失败的原因是数据内容非法，记录日志后继续；如果是记录重复，不>必记录日志，且继续。
