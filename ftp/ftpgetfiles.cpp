...
struct st_arg
{
  ...
  bool checktime;          // 是否需要检查服务端文件的时间，true-需要，false-不需要，缺省为false。
...
// 加载okfilename文件的内容到容器vlistfile1中。
bool LoadOKFile();
...
// 把ftp.nlist()方法获得到的list文件加载到容器vlistfile2中。
bool LoadListFile();
...
void _help()
{
  ...
  printf("<ptype>1</ptype> 文件下载成功后，远程服务器文件的处理方式：1-什么都不做；2-删除；3-备份，如果为3，还要指定备份的目录。\n");
  printf("<remotepathbak>/tmp/idc/surfdatabak</remotepathbak> 文件下载成功后，服务器文件的备份目录，此参数只有当type=3时才有效。\n");
  printf("<okfilename>/idcdata/ftplist/ftpgetfiles_surfdata.xml</okfilename> 已下载成功文件名清单，此参数只有当ptype=1时有效。\n");
  printf("<checktime>true</checktime> 是否需要检查服务端文件的时间，true-需要，false-不需要，此参数只有ptype=1时才有效，缺省为false。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  // 文件下载成功后，远程服务器文件的处理方式：1-什么都不做；2-删除；3-备份
  ...
  //是否需要检查服务端文件的时间，true-需要，false-不需要，此参数只有ptype=1时才有效，缺省为false。
  GetXMLBuffer(strxmlbuffer,"checktime",&starg.checktime);
