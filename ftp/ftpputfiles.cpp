...
bool _ftpputfiles()
{
  ...
  {
    logfile.Write("LoadLocalFile(%s) failed.\n"); return false;
  }
...
void _help()
{
  ...
  printf("Example:/project/tools1/tools1/bin/procctl 30 /project/tools1/bin/ftpputfiles /log/idc/ftpputfiles_surfdata.log \"<host>192.168.0.121:21</host><mode>1</mode><username>codelee</username><password>lixiang0222</password><localpath>/tmp/idc/surfdata</localpath><remotepath>/tmp/ftpputest</remotepath><matchname>SURF_ZH*.JSON</matchname><ptype>1</ptype><localpathbak>/tmp/idc/surfdatabak</localpathbak><okfilename>/idcdata/ftplist/ftpputfiles_surfdata.xml</okfilename><timeout>80</timeout><pname>ftpputfiles_surfdata</pname>\"\n\n\n");
  ...
  printf("<ptype>1</ptype> 文件上传成功后，本地文件的处理方式：1-什么都不做；2-删除；3-备份，如果为3，还要指定备份的目录。\n");
  printf("<lcoalpathbak>/tmp/idc/surfdatabak</localpathbak> 文件上传成功后，服务器文件的备份目录，此参数只有当type=3时才有效。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  GetXMLBuffer(strxmlbuffer,"lcoalpathbak",starg.localpathbak,300);   // 上传后客户端文件的备份目录  
